...
import type { ContestFeedbackPayload as FeedbackPayload } from "@/api/types";
...
/**
 * The platform-experience questionnaire, shared by contests and class tests.
 *
 * Both collect exactly the same answers so the CoE can compare across them without reconciling
 * two shapes. Everything surface-specific — which endpoints to call, where "back" goes, the
 * wording about what submitting unlocks — is injected.
 */
export interface FeedbackFormProps {
  /** Already-submitted check; the form redirects to `backTo` when this is true. */
  getStatus: () => Promise<{ submitted: boolean }>;
  submit: (payload: FeedbackPayload) => Promise<unknown>;
  statusQueryKey: readonly unknown[];
  /** Invalidated after a successful submit so the surface behind picks the change up. */
  invalidateQueryKey: readonly unknown[];
  pathname: string;
  backTo: string;
  backLabel: string;
  /** e.g. "contest" / "class test" — used in the intro sentence. */
  surfaceLabel: string;
  introSuffix: string;
...
export function FeedbackForm({
  ...
  const submitMutation = useMutation({
    mutationFn: (payload: FeedbackPayload) => submit(payload),
    onSuccess: async () => {
      toast.success(successMessage);
      setJustSubmitted(true);
      await queryClient.invalidateQueries({ queryKey: invalidateQueryKey });
      // Brief success beat before returning.
  ...
  // Already answered → nothing more to ask, so send them back.
      ...
      <div className="container max-w-3xl py-8">
        ...
        <Card className="animate-fade-in overflow-hidden border border-border bg-gradient-to-br from-accent/10 via-background to-background p-8 shadow-card motion-reduce:animate-none">
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            Tell us how the platform felt during this {surfaceLabel}. It takes under a minute
            {introSuffix}
          </p>
        </Card>
