...
class IsolateJob < ApplicationJob
  ...
  CGROUP_V2_CONTROLLERS_FILE = "/sys/fs/cgroup/cgroup.controllers".freeze
  ...
  def extract_archive
    ...
    #{memory_limit_flag(Config::MAX_MEMORY_LIMIT, submission.enable_per_process_and_thread_memory_limit)} \
  ...
  def run
    ...
    #{cg_timing_flag_for(submission)} \
    #{memory_limit_flag(submission.memory_limit, submission.enable_per_process_and_thread_memory_limit)} \
  ...
  def cgroup_v2_host?
    File.exist?(CGROUP_V2_CONTROLLERS_FILE)
  end

  def cgroup_flags_for_submission
    wants_cgroups = !submission.enable_per_process_and_thread_time_limit || !submission.enable_per_process_and_thread_memory_limit
    return "" unless wants_cgroups
    return "" if cgroup_v2_host?

    "--cg"
  end

  def memory_limit_flag(limit_kb, enable_per_process_limit)
    return "" unless cgroups.present?

    "#{enable_per_process_limit ? "-m " : "--cg-mem="}#{limit_kb}"
  end
